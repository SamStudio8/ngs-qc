...
\begin{itemize}
    ...
    \begin{itemize}
        \item Select each sample in turn...
        \item ...
        \item Call variants...
...
\section{Design}
\subsection{Purpose}
...as described briefly, \textbf{Goldilocks} is responsible for parsing input
files of the form displayed in ...
...positions are stored in memory per chromosome...

...once all the variants are loaded, \textbf{Goldilocks} is designed to search
over each chromosome... creating candiates as defined in the previous section...

..candidates are then ranked by criteria:

...for the purpose of this project we're looking at:
...the median of the GWAS (representative)
...the maximum of the iCHIP (enriching for maximum comparisons between data
sets)
...
\textbf{Goldilocks} is a modestly sized but elegantly
complex Python script which can also serve as a stand-alone module to be
imported in to other programs.

...whilst certainly more esoteric than \textbf{Frontier}, I still made the
choice to provide the option to import it as a module...
...perhaps a particular function will be of a use to an end user...
...with more time it would be interesting to research other potential uses for
such a script...


\subsection{Method}

...not quite quick and dirty but the initial codebase was overly complex...
...testing came second in terms of priority which was possibly unwise...

...with more time some of the functions would be more abstracted
...currently criteria are still hard coded to meet the needs of the project and
propose little re-use value unless altered...

...does however allow for repeatable results in future with different data sets
...
...simple file as shown in Appendix...
...
...however this would suffer from expensive memory moving operations and is ripe
for alteration...
...perhaps a set would be more appropriate...
