...
proposed input format for \textbf{Goldilocks}...
...
input files that contain line-delimited, colon-seperated chromosome and base
position entries (\textit{e.g.} 1:5000 is the base at position 5000 on
the first chromosome). Input files can be grouped, pooling the set of unique listed
...
\textbf{Goldilocks} requires the user provide their own valid "pathsfile" as
meeting the specification demonstrated in Section~\ref{sec:pathsfile} (an
example of such a file can be found in Appendix~\ref{app:pathsfile}).
