...
Given the unknown nature of input data, the number of observations and their
parameters are unknown before the input data is read and so memory cannot be
reserved to prevent expensive resizing operations.
...
allows the loading of data at any time...
... if the list were to require resizing
... to check for membership in a very large data set (as looking up a dict is
faster than a list)
...
The Statplexer is designed to provide the user methods for which to extract
